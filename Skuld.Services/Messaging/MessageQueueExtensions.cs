											 ...
											 int timeout = -1)
		{
			...
			switch (type)
			{
				case Models.MessageType.Standard:
					return await MessageSender.SendMessageTo(context.Channel, content, embed).ConfigureAwait(false);

				case Models.MessageType.Mention:
					return await MessageSender.SendMessageTo(context.Channel, $"{context.User.Mention} {content}", embed).ConfigureAwait(false);

				case Models.MessageType.DMS:
					return await MessageSender.SendMessageToUser(context.User, content, embed: embed).ConfigureAwait(false);

				case Models.MessageType.Timed:
					await MessageSender.SendMessageTo(context.Channel, content, embed, timeout: timeout).ConfigureAwait(false);
					return null;

				case Models.MessageType.File:
					{
						var msg = await MessageSender.SendMessageTo(context.Channel, content, embed, filestream, filename).ConfigureAwait(false);
				...
				case Models.MessageType.MentionFile:
					{
						var msg = await MessageSender.SendMessageTo(context.Channel, $"{context.User.Mention} {content}", embed, filestream, filename).ConfigureAwait(false);
