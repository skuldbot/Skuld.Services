...
namespace Skuld.Services.Gambling
{
	...
	{
		static readonly List<MiaSession> Sessions = new();
		...
		{
			StringBuilder str = new(isHidden ? "||" : "");
			...
			str.Append(
				string.Join(
					", ",
					dieArr.Select(x => isHidden ? "?" : x.Face.ToString(System.Globalization.CultureInfo.InvariantCulture)
				).ToArray()
				)
			);
